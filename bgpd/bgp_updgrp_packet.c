      ...
      if (paf->afi == AFI_IP && !peer_cap_enhe(peer))
	{
	  struct in_addr v4nh, *mod_v4nh;
          ...
          if (bgp_debug_update(peer, NULL, NULL, 0))
            zlog_debug ("u%llu:s%llu %s send UPDATE w/ nexthop %s",
	  ...
	  if (nhlen == 32)
	    {
          ...
          if (bgp_debug_update(peer, NULL, NULL, 0))
            {
              if (nhlen == 32)
                zlog_debug ("u%llu:s%llu %s send UPDATE w/ mp_nexthops %s, %s",
                ...
                zlog_debug ("u%llu:s%llu %s send UPDATE w/ mp_nexthop %s",
                            PAF_SUBGRP(paf)->update_group->id,
                            PAF_SUBGRP(paf)->id,
                            peer->host,
                            inet_ntop (AF_INET6, mod_v6nhg, buf, BUFSIZ));
