...
bgp_collision_detect (struct peer *new, struct in_addr remote_id)
{
  ...
  if ((peer = new->doppelganger) != NULL)
    {
      /* Do not accept the new connection in Established state or in any
       * other state where we cannot accept a new incoming connection -
       * Idle, Clearing, Deleted. Note that a peer GR is handled by
       * closing the existing connection upon receipt of new one.
       */
      if (peer->status == Established
	  || peer->status == Clearing)
        {
